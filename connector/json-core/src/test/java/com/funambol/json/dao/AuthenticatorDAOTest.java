...
public class AuthenticatorDAOTest extends AbstractHttpTransportTest
{
    private JsonAuthRequest authRequest = new JsonAuthRequest();
    private static final JsonServlet jsonServlet = new JsonServlet();
    private Converter<JsonAuthRequest> converter;
    
    public AuthenticatorDAOTest()
    {
        super(jsonServlet);
	...
	try {
	    ...
	    AuthenticatorDAO authenticator = new AuthenticatorDAOImpl("http://localhost:" +
                    com.funambol.json.util.Utility.PORT_DEFAULT + "/syncapi");
            
	    jsonServlet.setDoReturn(JsonServlet.LOGIN);
	    
	    String jsonObject = converter.toJSON(authRequest);
            
	    JsonResponse response = authenticator.login(jsonObject);
            
	    assertTrue(response.getResponseStatusCode() == 200);
	    assertTrue(response.getResponseBody().contains("sessionid"));
